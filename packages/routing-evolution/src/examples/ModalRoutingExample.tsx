...
function UserModal({ userId }: { userId: string }) {
  ...
  const users = {
    '1': { id: '1', name: 'Alice Johnson', role: 'Admin', email: 'alice@example.com', avatar: '👩‍💻' },
    '2': { id: '2', name: 'Bob Smith', role: 'Developer', email: 'bob@example.com', avatar: '👨‍💼' },
    '3': { id: '3', name: 'Carol Williams', role: 'Designer', email: 'carol@example.com', avatar: '👩‍🎨' }
  };
...
function ImageModal({ imageId }: { imageId: string }) {
  const navigate = useNavigate();
  const images = {
    '1': { id: '1', title: '山水风景', url: '🏔️', description: '美丽的山水风景照片' },
    '2': { id: '2', title: '城市夜景', url: '🌃', description: '繁华的城市夜景' },
    '3': { id: '3', title: '森林小径', url: '🌲', description: '宁静的森林小径' }
  };
...
function AccessibleModal({ children, onClose, title }) {
  ...
  return (
    <div className="min-h-full bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          >
            <div className="flex items-center mb-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">模态路由示例</h1>
          ...
          >
            <div className="card">
              <div className="card-header">
                <h2 className="text-xl font-semibold text-gray-900">🎭 模态演示</h2>
                <p className="text-sm text-gray-600 mt-1">点击按钮体验不同类型的模态路由</p>
              </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="text-sm space-y-1">
                    <div className="flex justify-between">
                      <span>模态状态:</span>
                      <span className={isModalRoute ? 'text-green-600' : 'text-gray-500'}>
                        {isModalRoute ? '已打开' : '已关闭'}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>当前路径:</span>
                      <code className="text-indigo-600 text-xs">{location.pathname}</code>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* 模态路由定义 */}
      <Routes>
        <Route path="/examples/modal/user/:userId" element={<UserModal userId={location.pathname.split('/').pop() || ''} />} />
        <Route path="/examples/modal/image/:imageId" element={<ImageModal imageId={location.pathname.split('/').pop() || ''} />} />
